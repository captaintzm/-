...
<script>
  ...
  export default {
    ...
    created () {
      // console.log(this.idser,'44444444444444')
    },
    created () {
    ...
    methods: {
      init (id) {
        ...
        this.$http({
          url: this.$http.adornUrl('/ybt-supply/sys/role/select'),
          method: 'get',
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/ybt-supply/sys/user/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
      ...
      dataFormSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ybt-supply/sys/user/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
